...
const CheckOutBankTransfer = ({ set, name,setorderID1 }) => {
  ...
  const handleSubmitBooking = () => {
    // FIXED: Move to success page after booking
    if (set) {
      set(2); // This should trigger success page in parent component
    } else {
      setblock(3); // Move to success page (step 3)
    }
  };
  ...
  return (
    <div className="flex gap-10">
      ...
      {/* STEP 3: Success Page - FIXED: Only show when block === 3 OR when set === 2 */}
      {(block === 3 || (set && block === 2)) && (
        <div className="w-full">
          <Successfullpayment
            Message={
            ...
            orderid={orderID ? `Your Order ID: ${orderID}` : ""}
          />
        </div>
      )}
